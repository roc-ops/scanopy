...
//! whose NIC actually heard a neighbour gain neighbour evidence. It does not report the host's own
...
//! gone this is a silent no-op and the stored LLDP evidence is preserved, exactly as it is when
//! an SNMP credential is removed.
//!
...
/// lldpcli lists each as its own entry under the same interface name. Each becomes its own
/// evidence entry on the row, as the SNMP path does for its remote table (GH #701).
pub(super) fn apply_neighbors(interfaces: &mut [Interface], neighbors: Vec<LldpdNeighbor>) {
    for n in neighbors {
        ...
        row.base
            .neighbor_candidates
            .push(InterfaceNeighborEvidence {
                lldp_chassis_id: n.chassis_id,
                lldp_port_id: n.port_id,
                lldp_sys_name: n.sys_name,
                lldp_port_desc: n.port_desc,
                lldp_mgmt_addr: n.mgmt_addr,
                lldp_sys_desc: n.sys_desc,
                ..Default::default()
            });
    }
}
...
mod tests {
    ...
    fn neighbors_decorate_existing_rows_only() {
        ...
        assert!(rows[0].base.neighbor_candidates.is_empty());
        assert!(rows[2].base.neighbor_candidates.is_empty());

        let [eno1] = rows[1].base.neighbor_candidates.as_slice() else {
            panic!("eno1 should carry exactly one neighbour");
        };
        assert_eq!(
    ...
    /// Two neighbours heard on the same port: both are kept, in lldpcli's order.
    #[test]
    fn several_neighbours_on_one_port_are_all_kept() {
        ...
        let names: Vec<_> = rows[0]
            ...
            .neighbor_candidates
            .iter()
            .map(|e| e.lldp_sys_name.as_deref())
            .collect();

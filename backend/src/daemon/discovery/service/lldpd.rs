...
impl LldpdError {
    pub(super) fn outcome(&self) -> AttemptOutcome {
        match self {
            ...
            // lldpd was reached and the read still failed; its stderr says why.
            Self::Failed { .. } => AttemptOutcome::CollectionFailed,
...
pub(super) fn parse_neighbors(json: &Value) -> Vec<LldpdNeighbor> {
    let Some(interfaces) = json.get("lldp").and_then(|l| l.get("interface")) else {
        return Vec::new();
    };
    let mut out = Vec::new();
...
mod tests {
    ...
    /// Trimmed from a real `lldpcli -f json show neighbors details` on netlab-server
    /// (Ubuntu 26.04, lldpd 1.0.x, 2026-08-25): an ifname-typed port, a local-typed port
    /// with a port description, and a multi-address chassis.
    ...
    fn parses_neighbors_with_both_port_id_shapes() {
        let neighbors = parse_neighbors(&serde_json::from_str(NEIGHBORS).unwrap());
    ...
    #[test]
    fn single_neighbor_object_and_absent_sections_parse() {
        ...
        let neighbors = parse_neighbors(&serde_json::from_str(single).unwrap());
        assert_eq!(neighbors.len(), 1);
        assert_eq!(neighbors[0].local_interface, "eth0");

        assert!(
            parse_neighbors(&serde_json::from_str::<Value>(r#"{"lldp":{}}"#).unwrap()).is_empty()
        );
    }
    ...
    fn several_neighbours_on_one_port_keep_the_first() {
        ...
        let neighbors = parse_neighbors(&serde_json::from_str(two).unwrap());
        assert_eq!(neighbors.len(), 2);

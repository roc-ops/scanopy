...
mod tests {
    ...
    fn every_device_has_its_own_address_and_name() {
        ...
        for device in &lab {
            ...
            assert!(
                // Floor lowered from 230 to 227 for GH #701's three devices, then to 225 for
                // GH #668's last two (`switch-dlink-02`, `pc-windows-nic-filters`): .227-.254 (28
                // addresses) were already fully occupied, with .255 unusable (broadcast on this
                // /22) — there was no room left above .227.
                octets[..3] == [192, 168, 7] && octets[3] >= 225,
